...
Or run with the static inventory generated by the role (written to `hcloud_inventory_path`, default `/tmp/hcloud_inventory.ini`), or adapt `examples/inventory/hcloud_static_example.ini` as a starting point:

```bash
ansible-playbook -i /tmp/hcloud_inventory.ini site.yml
```
